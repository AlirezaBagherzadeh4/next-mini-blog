...
export const BlogCard: React.FC<IPost> = ({
  ...
  return (
    <Link
      href={routes.post(id)}
      target="_blank"
      className="group flex h-full w-full flex-col items-center justify-between gap-4 rounded-lg border border-gray-200 transition-shadow duration-300 hover:shadow-sm"
    >
      <div className="flex h-fit w-full flex-col items-center justify-between gap-4">
        <div className="h-full max-h-80 w-full overflow-hidden rounded-t-lg">
          <Image
            src={image}
            alt={title}
            width={800}
        ...
        <div className="flex h-fit w-full flex-col items-center justify-center gap-4 px-4 pb-4">
      ...
      <span className="self-end pr-2 pb-2 text-base text-blue-500 group-hover:underline">
        Read more
      </span>
    </Link>
  );
